...
public class CrazyDigitalPainting {
	...
	Color[][] ArrayColor = new Color[width][height];
	...
	public CrazyDigitalPainting() {
		...
		for (int i = 0; i < ArrayColor.length; i++) {
			for (int j = 0; j < ArrayColor[i].length; j++) {
				...
				ArrayColor[i][j] = new Color(j % 256, (i * i) % 256, j % 256);
				System.out.println(ArrayColor[i][j]); // printing the elements
			}
		...
		c.displayColorsAsImage(ArrayColor);
		c.getClass();
